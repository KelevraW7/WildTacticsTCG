Shader "Wobblewares/WobbleFontTMP" {
...
SubShader{
	...
	Pass {
		...
		v2f vert (appdata_t v)
		{
			...
			uint row = floor( index / Columns);
			uint col = index - row * Columns;

			float OffsetPerColumn = 1.0f / (float)Columns;
			float OffsetPerRow = 1.0f / (float)Rows;

			// adjust text coord based on current frame
			float2 alteredTexCoord;
			alteredTexCoord.x = v.texcoord0.x * OffsetPerColumn + col * OffsetPerColumn;
			alteredTexCoord.y = v.texcoord0.y * OffsetPerRow	   + row * OffsetPerRow;
			OUT.texcoord0 = alteredTexCoord;
